...
class ArcBallT:
	...
	def drag (self, NewPt):
		...
		if (Vector3fLength(Perp) > Epsilon):		#    //if its non-zero
			...
			#Tcll5850: fixed improper method usage (list.index -> list[index])
			#NewRot is an iterator, not a class
			NewRot[X] = 0.0
			NewRot[Y] = 0.0
			NewRot[Z] = 0.0
			NewRot[W] = 0.0
			#Tcll5850: python doesn't cooperate well when using vars set with each other
			#(whenever one's updated, they're all updated)
