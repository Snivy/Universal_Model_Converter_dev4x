...
def Case(compair): global __sw; return __sw==compair
#provided for conveinence:
def switch(value): Switch(value)
def case(compair): global __sw; return __sw==compair
...
__default_encoding='ascii'

def String(value=None,code=__default_encoding,label=''):
	...
	if value==None: #read string to 0x00
		...
		while IWLD(True) or __o[__c]<=(len(__f[__c])-1): #EOF? (lazy... need better EOF checking)
			...
			if c==0: break
			else: STRING='%s%s'%(STRING,__ch__[c].decode(code))
		__LOG("%s: read %s encoded string '%s' to chr(0)%s"%(p,code,STRING,label))
		return STRING
	elif type(value)==int:
		if value==0: #read entire file
			STRING=[__ch__[i].decode(code) for i in __f[__c]]
			__LOG("%s: read file as string"%p)
			#__LOG(''.join(STRING))
			return ''.join(STRING)
		elif __o[__c]+value<=(len(__f[__c])-1): #read length, EOF?
			STRING=__STR(__f[__c][__o[__c]:__o[__c]+value]).decode(code)
			__o[__c]+=value
			__LOG("%s: read %s encoded string '%s' of length %i%s"%(p,code,STRING,value,label))
			return STRING
	elif type(value)==str: #write string
		__f[__c]+=[ord(c) for c in value.encode(code)]
		__o[__c]+=len(value.encode(code))
		__LOG(("%s: wrote string '%s'%s"%(p,value,label)).replace('\n','\\n'))
...
#provided for conveinence:
def string(value=None,code=__default_encoding,label=''): return String(value,code,label)
...
def MtxMultiply(Mtx1,Mtx2,MatchType=1):
    #TODO: equivalate matrix types
    # ^ output matrix will match the matrix specified by 'MatchType' (either 1 or 2)
    # ( Mtx1=[4x3], Mtx2=[4x4], MatchType=1 >>> [4x3]
    return [[(Mtx1[0][0]*Mtx2[0][0]) + (Mtx1[0][1]*Mtx2[1][0]) + (Mtx1[0][2]*Mtx2[2][0]) + (Mtx1[0][3]*Mtx2[3][0]),
             (Mtx1[0][0]*Mtx2[0][1]) + (Mtx1[0][1]*Mtx2[1][1]) + (Mtx1[0][2]*Mtx2[2][1]) + (Mtx1[0][3]*Mtx2[3][1]),
             (Mtx1[0][0]*Mtx2[0][2]) + (Mtx1[0][1]*Mtx2[1][2]) + (Mtx1[0][2]*Mtx2[2][2]) + (Mtx1[0][3]*Mtx2[3][2]),
             (Mtx1[0][0]*Mtx2[0][3]) + (Mtx1[0][1]*Mtx2[1][3]) + (Mtx1[0][2]*Mtx2[2][3]) + (Mtx1[0][3]*Mtx2[3][3])],
            [(Mtx1[1][0]*Mtx2[0][0]) + (Mtx1[1][1]*Mtx2[1][0]) + (Mtx1[1][2]*Mtx2[2][0]) + (Mtx1[1][3]*Mtx2[3][0]),
             (Mtx1[1][0]*Mtx2[0][1]) + (Mtx1[1][1]*Mtx2[1][1]) + (Mtx1[1][2]*Mtx2[2][1]) + (Mtx1[1][3]*Mtx2[3][1]),
             (Mtx1[1][0]*Mtx2[0][2]) + (Mtx1[1][1]*Mtx2[1][2]) + (Mtx1[1][2]*Mtx2[2][2]) + (Mtx1[1][3]*Mtx2[3][2]),
             (Mtx1[1][0]*Mtx2[0][3]) + (Mtx1[1][1]*Mtx2[1][3]) + (Mtx1[1][2]*Mtx2[2][3]) + (Mtx1[1][3]*Mtx2[3][3])],
            [(Mtx1[2][0]*Mtx2[0][0]) + (Mtx1[2][1]*Mtx2[1][0]) + (Mtx1[2][2]*Mtx2[2][0]) + (Mtx1[2][3]*Mtx2[3][0]),
             (Mtx1[2][0]*Mtx2[0][1]) + (Mtx1[2][1]*Mtx2[1][1]) + (Mtx1[2][2]*Mtx2[2][1]) + (Mtx1[2][3]*Mtx2[3][1]),
             (Mtx1[2][0]*Mtx2[0][2]) + (Mtx1[2][1]*Mtx2[1][2]) + (Mtx1[2][2]*Mtx2[2][2]) + (Mtx1[2][3]*Mtx2[3][2]),
             (Mtx1[2][0]*Mtx2[0][3]) + (Mtx1[2][1]*Mtx2[1][3]) + (Mtx1[2][2]*Mtx2[2][3]) + (Mtx1[2][3]*Mtx2[3][3])],
            [(Mtx1[3][0]*Mtx2[0][0]) + (Mtx1[3][1]*Mtx2[1][0]) + (Mtx1[3][2]*Mtx2[2][0]) + (Mtx1[3][3]*Mtx2[3][0]),
             (Mtx1[3][0]*Mtx2[0][1]) + (Mtx1[3][1]*Mtx2[1][1]) + (Mtx1[3][2]*Mtx2[2][1]) + (Mtx1[3][3]*Mtx2[3][1]),
             (Mtx1[3][0]*Mtx2[0][2]) + (Mtx1[3][1]*Mtx2[1][2]) + (Mtx1[3][2]*Mtx2[2][2]) + (Mtx1[3][3]*Mtx2[3][2]),
             (Mtx1[3][0]*Mtx2[0][3]) + (Mtx1[3][1]*Mtx2[1][3]) + (Mtx1[3][2]*Mtx2[2][3]) + (Mtx1[3][3]*Mtx2[3][3])]]
